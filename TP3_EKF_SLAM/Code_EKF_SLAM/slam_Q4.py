...
Py_sim = (1 * np.diag([np.deg2rad(5)])) ** 2
    ...
    for iy in range(len(y[:, 0])):  # for each observation
        ...
        if min_id == nLM:
            ...
            for iRayInit in range(Ng):
                ...
                bottomPart = np.hstack((Jr @ PEst[0:3, 0:3], Jr @ PEst[0:3, 3:]))
                rightPart = bottomPart.T
                PEst = np.vstack((np.hstack((PEst, rightPart)), np.hstack((bottomPart, Jr @ PEst[0:3, 0:3] @ Jr.T + Jy @ Py_hypo @ Jy.T))))
        else:
...
def main():
    ...
    # Landmarks = np.array([[0.0, 5.0]])
    # for i in range(N):
    #     Landmarks = np.append(Landmarks, np.array([[0.0 + R1 * math.cos((2 * i + 1) * math.pi / N), 10.0 + R1 * math.sin((2 * i + 1) * math.pi / N)]]), axis=0)
    # for i in range(N):
    #     Landmarks = np.append(Landmarks, np.array([[0.0 + R2 * math.cos(2 * i * math.pi / N), 10.0 + R2 * math.sin((2 * i) * math.pi / N + 0.1)]]), axis=0)
    # Landmarks = Landmarks[1:, :]

    # Q1_3， yaw_rate= 0.1
    # Define landmark positions [x, y]
    # Landmarks = np.array([[0.0, 2.0],
    #                       [2.0, 0.2],
    #                       [-1.0, -0.5],
    #                       [0.0, -1.5]])
